...
REVIEW_PROMPT = (
    ...
    "随后给你从一段 AI 生成视频抽出的若干帧（按时间顺序）。这段视频是在「简洁室内、中性灰背景」的"
    "中性场景下生成的，用于评级动作模板的可用性，请只审查动作迁移质量、不评画面美感。严格审查：\n"
    "1) character_locked：画面中的角色是否与参考图保持同一人（东方面孔、黑短发、细框眼镜、深灰大衣），"
    "还是变成了参考动作视频里的人；\n"
    "2) motion_natural：是否复刻了参考动作（自然流畅的肢体运动），还是僵着不动只推镜；\n"
    "3) spatial_stable：运镜是否舒服，背景（简洁室内、中性灰背景）是否稳定不漂移、不扭曲。\n"
